...
class CRUD extends Simulation {
  ...
  val postScn: ScenarioBuilder = scenario("Post").exec(
    http("Post").post("/v2/pet")
      .body(RawFileBody("TestData/pet.json")).asJson
  )
  val getScn: ScenarioBuilder = scenario("Get").exec(
    http("Get").get("/v2/pet/710")
  )
  val putScn: ScenarioBuilder = scenario("Put").exec(
    http("Put").put("/v2/pet")
      .body(RawFileBody("TestData/updatedPet.json")).asJson
  )
  val deleteScn: ScenarioBuilder = scenario("Delete").exec(
